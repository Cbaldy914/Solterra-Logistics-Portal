        ...
        <div class="accordion-section" data-section="3">
            ...
            <div class="accordion-content">
                ...
                <div class="detailed-mode-container" id="detailedModeContainer">
                    ...
                    <!-- Copy movements to other warehouses -->
                    <div class="copy-movements-container" id="copyMovementsContainer" style="display: none;">
                        <div class="copy-movements-box">
                            <div class="copy-movements-header">
                                <span class="copy-icon">📋</span>
                                <span>Copy Pallet Movements</span>
                            </div>
                            <div class="copy-movements-content">
                                <label for="sourceWarehouseSelect">Copy from:</label>
                                <select id="sourceWarehouseSelect" class="source-warehouse-select">
                                    <!-- Populated by JavaScript -->
                                </select>
                                <button type="button" class="btn-copy-movements" onclick="copyMovementsToAllWarehouses()">
                                    Copy to All Other Warehouses
                                </button>
                            </div>
                        </div>
                    </div>
...
function renderDetailedMode() {
    ...
    const copyContainer = document.getElementById('copyMovementsContainer');
    const sourceSelect = document.getElementById('sourceWarehouseSelect');
    ...
    // Show/hide copy movements section based on warehouse count
    if (calculatorData.warehouses.length > 1) {
        ...
        let selectHtml = '';
        calculatorData.warehouses.forEach((wh, index) => {
            selectHtml += `<option value="${index}">${escapeHtml(wh.name)}</option>`;
        });
...
function copyMovementsToAllWarehouses() {
    const sourceSelect = document.getElementById('sourceWarehouseSelect');
    const sourceIndex = parseInt(sourceSelect.value);
    const sourceWarehouse = calculatorData.warehouses[sourceIndex];
    ...
    // Deep copy the movements from source to all other warehouses
    const sourceMovements = JSON.parse(JSON.stringify(sourceWarehouse.movements || {}));

    let copiedCount = 0;
    calculatorData.warehouses.forEach((wh, index) => {
        if (index !== sourceIndex) {
            wh.movements = JSON.parse(JSON.stringify(sourceMovements));
            copiedCount++;
        }
    });
    ...
    alert(`Pallet movements copied from "${sourceWarehouse.name}" to ${copiedCount} other warehouse${copiedCount !== 1 ? 's' : ''}.`);
}
